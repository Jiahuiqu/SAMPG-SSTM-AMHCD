...
def points_generation(dataset_name='China', train_ratio=0.2, val_ratio=0.3, label_ratio=0.01):
    ...
    img_t1, img_t2, img_gt = get_dataset(dataset_name, '0')
    ...
    if dataset_name == 'China' or dataset_name == 'River':
        changed_indices = np.where(whole_point[0] == 1.)[0].tolist()
        unchanged_indices = np.where(whole_point[0] == 0.)[0].tolist()
    elif dataset_name == 'BayArea' or dataset_name == 'Barbara' or dataset_name == 'Hermiston':
    ...
    return (img_gt, train_points, labeled_points, unlabeled_points, val_points, changed_points_labeled,
            unchanged_points_labeled)


def save_points_to_file(points, dataset_name, mode):
    file_name = os.path.join(f"../datasets/{dataset_name}", f"{mode}_wr_1%.txt")   #wr:well reg   bx:旋转角度    sj：随机变形
...
if __name__ == '__main__':
    dataset_name = 'Hermiston'   # China, Hermiston, River, BayArea, Barbara, Liyukou
    train_ratio = 0.008
    val_ratio =  0.002
    label_ratio = 0.001

    (img_gt, train_points, labeled_points, unlabeled_points, val_points, changed_points_labeled,
     unchanged_points_labeled) = points_generation(dataset_name, train_ratio, val_ratio, label_ratio)
